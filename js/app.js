(() => {
    ...
    function windowLoad() {
        ...
        menuInit();
        if (isMobile.any()) document.querySelector("body").classList.add("touch");
        const mainBlock = document.querySelectorAll(".menu-header__link")[1];
        const about = document.querySelectorAll(".menu-header__link")[2];
        if (document.querySelector("body").hasAttribute("data-active")) ; else {
            mainBlock.classList.remove("menu-header__link_active");
            about.classList.add("menu-header__link_active");
        }
        if ("uk" === saveUserLang || "en" === saveUserLang) var userLang = saveUserLang; else userLang = navigator.language;
        ...
        if (langButton) langButton.addEventListener("click", (function(e) {
            chageLang(true);
        }));
